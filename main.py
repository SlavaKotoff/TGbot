...
QIWI_TOKEN = '72daf03fba5171d3421ca7a071956703'
QIWI_WALLET = '+79885356314'
...
🔞Добро пожаловать в <b>SLIV!</b>🔞
📸 <b>СЛИТЫЕ ФОТО</b> всего за <b>10 руб.</b>
🎥 <b>ПРИВАТНЫЕ ВИДЕО</b> за <b>50 руб.</b>
🤖 <b>БОТ</b> имеет <b>ОГРОМНУЮ</b> базу данных из более чем <b>50.000 фото</b> и <b>10.000 видео</b>

===============================

💵 <b>Ваш Баланс:</b> {_data[5]}
👤 <b>Приглашено:</b> {db.get_refs(user_id)}
🔥 <b>Зарабатывай по {db.get_settings()[5]}₽ за каждого приглашенного друга по этой ссылке: t.me/{link}?start={user_id}</b>
"""
...
def check_payments():
    ...
    response = requests.get(f"https://edge.qiwi.com/payment-history/v2/persons/{QIWI_WALLET}/payments", headers=headers)
    ...
    for payment in payments:
        ...
        if status == 'SUCCESS':
            # Предположим, что комментарий к платежу является ID пользователя
            user_id = int(comment)
            # Начислите пользователю баланс на основе суммы платежа
            current_balance = db.get_balance(user_id)
            db.set_balance(user_id, current_balance + amount)
